...
{
    solveGyroscopeBias(all_image_frame, Bgs, base_cam); 
#if USE_DEPTH_INITIAL
    if (LinearAlignmentWithDepth(all_image_frame, g, x, base_cam)) 
#else
    if (LinearAlignment(all_image_frame, g, x, base_cam)) 
#endif
        return true;
    else
        return false;
